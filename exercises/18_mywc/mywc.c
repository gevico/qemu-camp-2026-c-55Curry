...
void add_word(WordCount **hash_table, const char *word) {
    ...
    WordCount *curr = hash_table[hash_word];
    ...
    new_node->next = hash_table[hash_word];
    hash_table[hash_word] = new_node;
}
...
// 释放哈希表内存
void free_hash_table(WordCount **hash_table) { free(hash_table); }
